...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_user_progress'
    ) THEN
        EXECUTE $$
...
CREATE UNIQUE INDEX IF NOT EXISTS mv_user_progress_user_id_idx ON mv_user_progress (user_id);

COMMENT ON MATERIALIZED VIEW mv_user_progress IS 'Refresh to sync XP/levels after task logs; consider REFRESH CONCURRENTLY in prod.';
