...
-- ============================================================================
-- Script: 090_refresh_helpers.sql
-- Objetivo: Crear procedimientos para refrescar materialized views en orden seguro.
-- ============================================================================

BEGIN;

-- --------------------------------------------------------------------------
-- Refresca la vista materializada diaria asegurando su existencia.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Refresca la vista materializada semanal dependiente de la diaria.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Procedimiento maestro que refresca en el orden correcto.
-- --------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE public.sp_full_refresh_all()
LANGUAGE plpgsql
AS $$
BEGIN
  PERFORM pg_advisory_xact_lock(hashtext('sp_full_refresh_all'));
  CALL public.sp_refresh_daily_materialized();
  CALL public.sp_refresh_weekly_materialized();
END;
$$;
