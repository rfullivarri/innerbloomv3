      ...
      {items.map((item) => {
        ...
        return (
          <motion.button
            ...
            className={`flex w-full min-w-0 items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-left text-sm text-white/80 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-300/60 hover:border-white/25 hover:bg-white/10 data-[selected=true]:border-sky-400/70 data-[selected=true]:bg-sky-400/10 data-[selected=true]:text-white data-[selected=true]:shadow-inner data-[selected=true]:shadow-white/25 data-[disabled=true]:cursor-not-allowed data-[disabled=true]:opacity-60 data-[disabled=true]:hover:border-white/10 data-[disabled=true]:hover:bg-white/5`}
          >
            <SelectableCheck
              selected={active}
              toneClassName="data-[selected=true]:border-transparent data-[selected=true]:bg-sky-400 data-[selected=true]:text-slate-900"
            />
            <span className="flex-1 min-w-0 break-words pr-1">{item}</span>
          </motion.button>
        );
      })}
    </div>
