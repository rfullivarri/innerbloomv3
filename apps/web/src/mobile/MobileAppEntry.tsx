...
function NativeWelcomeCarousel({ language }: { language: 'es' | 'en' }) {
  ...
  const activeSlideDuration = activeIndex === 3 ? 9000 : NATIVE_WELCOME_SLIDE_MS;
  ...
  }, [activeIndex, activeSlideDuration, slides.length]);
  ...
  return (
    <section className="native-welcome-carousel" aria-live="polite" data-native-step={activeIndex + 1}>
      ...
      >
        <LandingV3MethodVisual index={activeIndex} language={language} logrosCycleMs={activeIndex === 3 ? activeSlideDuration : undefined} nativePreview />
      </div>
      <div className="native-welcome-carousel-controls" aria-label="Carousel progress" role="tablist">
        {slides.map((slide, index) => (
          ...
          >
            {index === activeIndex ? (
              <i
                style={{ animationDuration: `${activeSlideDuration}ms` }}
