...
import { buildSuppliedAssetCodexTask, extractDriveFolderId, type SuppliedDriveAsset } from '../../lib/marketingSuppliedAssetTask';
...
export function MarketingPageV2() {
  ...
  const [suppliedAssets, setSuppliedAssets] = useState<SuppliedDriveAsset[]>([]);
  ...
  async function addAsset(postId: string, file: File) {
    ...
    if (post.format === 'story' && post.assets.length >= 1) return setError('An Instagram Story uses exactly one image.');
    if (post.format !== 'story' && post.assets.length >= MAX_CAROUSEL_ASSETS) return setError('Instagram carousels support a maximum of 10 images.');
  ...
  async function inspectSuppliedAssets() {
    const folderId = extractDriveFolderId(driveFolder);
    if (!folderId) return setError('Paste a Google Drive folder URL or its folder ID.');
  ...
  async function copySuppliedAssetTask() {
    if (!selectedCampaign || !suppliedAssets.length) return;
    const prompt = buildSuppliedAssetCodexTask({ campaignCode: `ib_${new Date().toISOString().slice(0, 7).replace('-', '')}_stories`, title: `${selectedCampaign.title} — supplied Stories`, driveFolderId: extractDriveFolderId(driveFolder), format: 'story', scheduleStart: new Date().toISOString().slice(0, 10), strategyCampaign: selectedCampaign, assets: suppliedAssets });
    ...
    setTaskCopied(true); setMessage('Task copied. Paste it into Codex Cloud; it will create a PR, not run an API agent.');
  }
  ...
  return <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-5 pb-16">
    ...
    {loading ? <LoadingPanel/> : tab === 'posts' ? <>
      <section className="rounded-2xl border border-[color:var(--admin-border)] bg-[color:var(--admin-surface)] p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[color:var(--admin-muted)]">Supplied visual bases</p><h3 className="mt-1 text-lg font-semibold">Create a Codex Story task</h3><p className="mt-1 max-w-3xl text-sm text-[color:var(--admin-muted)]">Select a Drive folder and this strategy campaign. The button only copies a task for Codex Cloud; it never calls an OpenAI API or uploads to R2.</p></div><button className="admin2-btn admin2-btn--primary" disabled={!suppliedAssets.length} onClick={() => void copySuppliedAssetTask()}>{taskCopied ? 'Copied' : 'Copy task for Codex'}</button></div><div className="mt-4 flex flex-wrap gap-2"><input value={driveFolder} onChange={(event) => { setDriveFolder(event.target.value); setSuppliedAssets([]); setTaskCopied(false); }} placeholder="Google Drive folder URL or ID" className="min-w-[280px] flex-1 rounded-xl border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-muted)] px-3 py-2 text-sm"/><button className="admin2-btn admin2-btn--secondary" disabled={inspectingAssets || !driveFolder.trim()} onClick={() => void inspectSuppliedAssets()}>{inspectingAssets ? 'Checking Drive…' : 'Inspect images'}</button></div>{suppliedAssets.length ? <p className="mt-3 text-sm text-emerald-200">{suppliedAssets.length} image bases found · one 1080×1920 Story per image · R2 only after Admin approval.</p> : null}</section>
...
function ReviewCard({ post, expanded, editing, dirty, uploadingAsset, onToggle, onEdit, onDone, onChange, onStatus, onRemoveAsset, onAddAsset }: { post: EditablePost; expanded: boolean; editing: boolean; dirty: boolean; uploadingAsset: boolean; onToggle: () => void; onEdit: () => void; onDone: () => void; onChange: (updater: (post: EditablePost) => EditablePost) => void; onStatus: (status: EditablePost['status']) => void; onRemoveAsset: (index: number) => void; onAddAsset: (file: File) => void; }) {
  ...
  return <article className={`overflow-hidden rounded-2xl border bg-[color:var(--admin-surface)] transition ${post.status === 'approved' ? 'border-emerald-500/45' : 'border-[color:var(--admin-border)]'}`}>
    ...
    {expanded ? <div className="space-y-5 p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold">{post.format === 'story' ? 'Story asset' : 'Carousel assets'}</p><p className="text-xs text-[color:var(--admin-muted)]">{post.assets.length}/{post.format === 'story' ? 1 : MAX_CAROUSEL_ASSETS} images</p></div>{editing ? <label className={`admin2-btn admin2-btn--secondary inline-flex cursor-pointer items-center gap-2 ${uploadingAsset ? 'pointer-events-none opacity-60' : ''}`}>{uploadingAsset ? <Spinner size="sm"/> : <ImagePlus size={16}/>} {uploadingAsset ? 'Uploading' : 'Add image'}<input type="file" accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={handleFile} disabled={uploadingAsset || (post.format === 'story' ? post.assets.length >= 1 : post.assets.length >= MAX_CAROUSEL_ASSETS)}/></label> : null}</div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{post.assets.map((asset, index) => <figure key={`${asset.file}-${index}`} className="group relative overflow-hidden rounded-xl border border-[color:var(--admin-border)] bg-[color:var(--admin-surface-muted)]"><img src={asset.previewUrl || asset.url} alt={asset.title} className={post.format === 'story' ? 'aspect-[9/16] w-full object-cover' : 'aspect-square w-full object-cover'}/>{editing ? <button type="button" aria-label={`Remove ${asset.file}`} onClick={() => onRemoveAsset(index)} className="absolute right-2 top-2 inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/70 text-red-200 hover:bg-red-500 hover:text-white"><Trash2 size={17}/></button> : null}<figcaption className="flex items-center justify-between gap-2 p-3 text-xs text-[color:var(--admin-muted)]"><span className="truncate">{asset.file}</span><span>{index + 1}/{post.assets.length}</span></figcaption></figure>)}</div>
...
function mapPost(post: MarketingPostRecord): EditablePost { const [date = '', rawTime = '19:30:00'] = (post.scheduledAt ?? '').split('T'); const format = post.format === 'carousel' || post.format === 'story' || post.format === 'reel' ? post.format : 'static'; return { id: post.postCode, number: post.postCode.replace(/^post_?/, '').padStart(3, '0'), platform: 'instagram', format, status: post.status === 'approved' || post.status === 'draft' ? post.status : 'needs_review', scheduledDate: date, scheduledTime: rawTime.slice(0,8) || '19:30:00', hypothesis: post.hypothesis, metric: post.targetMetric, caption: post.caption, trackingUrl: post.trackingUrl, assets: post.assetUrls.map(mapAsset) }; }
