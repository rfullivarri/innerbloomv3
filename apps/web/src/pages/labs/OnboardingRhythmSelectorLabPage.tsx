...
function buildSinePath(option: RhythmOption) {
...
function RhythmWave({ option, selected, theme }: { option: RhythmOption; selected: boolean; theme: RhythmTheme }) {
  ...
  const nodeGradientId = `rhythm-node-gradient-${rawId}`;
  ...
  return (
    <svg className="h-[3.35rem] w-full min-w-[8.25rem] overflow-visible sm:h-16" viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} role="img" aria-label={`${option.name} weekly rhythm wave`} preserveAspectRatio="none">
      <defs>
        ...
        <radialGradient id={nodeGradientId} cx="50%" cy="45%" r="62%">
      ...
      <circle cx={node.x} cy={node.y} r={selected ? 28 : 22} fill={`url(#${nodeGradientId})`} opacity={selected ? (theme === 'light' ? 0.17 : 0.28) : theme === 'light' ? 0.1 : 0.16} filter="blur(3px)" />
      <circle cx={node.x} cy={node.y} r={selected ? 18 : 15} fill={`url(#${nodeGradientId})`} opacity={theme === 'light' ? 0.92 : 0.98} />
      <circle cx={node.x - 4} cy={node.y - 10} r={4.3} fill={theme === 'light' ? 'rgba(255,255,255,0.72)' : 'rgba(214,211,255,0.62)'} />
    </svg>
  );
}

function RhythmWaveSelectorPreview({ theme }: { theme: RhythmTheme }) {
    ...
    >
      <div className="space-y-3 md:space-y-4">
        {RHYTHM_OPTIONS.map((option) => {
          const selected = option.name === 'Flow';
          return (
            <article
              key={option.name}
              className={`relative grid min-h-[4.75rem] grid-cols-[4.2rem_minmax(0,1fr)_4.7rem] items-center gap-2 overflow-hidden rounded-[1.45rem] border px-3 py-2 transition sm:grid-cols-[5.5rem_minmax(0,1fr)_6.5rem] sm:gap-4 sm:px-5 md:min-h-[6.05rem] md:grid-cols-[7rem_minmax(0,1fr)_7.5rem] md:rounded-[1.8rem] md:px-6 ${
                selected
                  ? isLight
                  ...
                  : isLight
                    ? 'border-[#ded5f4]/84 bg-[linear-gradient(135deg,rgba(255,255,255,0.70),rgba(247,243,255,0.58))] shadow-[inset_0_1px_0_rgba(255,255,255,0.82)]'
                    : 'border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,0.055),rgba(10,12,27,0.48))] shadow-[inset_0_1px_0_rgba(255,255,255,0.055)]'
              }`}
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between gap-3 px-1">
        ...
        <span className={`rounded-full border px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.18em] ${isLight ? 'border-[#d8cff3] bg-white/60 text-[#5a4a7a]' : 'border-white/12 bg-white/[0.06] text-white/62'}`}>Flow selected</span>
      </div>
      <div className="rounded-[2.4rem] p-3 sm:p-5 md:p-7" style={getThemeSurface(theme)}>
        <RhythmWaveSelectorPreview theme={theme} />
      </div>
    </section>
  );
}
