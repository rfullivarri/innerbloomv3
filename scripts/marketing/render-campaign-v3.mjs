...
.brand{position:absolute;top:64px;left:68px;z-index:20}.brand img{display:block;width:238px;height:46px;object-fit:contain;object-position:left center}.brand.mark img{width:52px;height:52px;object-position:center}
...
.device{position:absolute;z-index:8;width:332px;height:686px;padding:10px;border-radius:58px;background:linear-gradient(145deg,#56525c 0%,#17161a 21%,#020204 52%,#4e4b53 100%);box-shadow:0 48px 110px rgba(18,10,35,.35),inset 0 0 0 1px rgba(255,255,255,.38)}
...
.screen{position:relative;width:100%;height:100%;overflow:hidden;border-radius:49px;background:#08080a}.screen img{display:block;width:100%;height:100%;object-fit:cover;object-position:top center}.island{position:absolute;z-index:4;top:17px;left:50%;transform:translateX(-50%);width:105px;height:28px;border-radius:20px;background:#020204;box-shadow:inset 0 0 0 1px rgba(255,255,255,.06)}.shine{position:absolute;z-index:3;inset:2px;border-radius:54px;background:linear-gradient(112deg,rgba(255,255,255,.18),transparent 20%,transparent 72%,rgba(255,255,255,.06));pointer-events:none}
.device.small{width:270px;height:558px;border-radius:48px}.device.small .screen{border-radius:40px}.device.small .island{width:82px;height:22px;top:14px}
...
.layered-depth .copy{left:68px;top:240px;width:500px}.layered-depth .copy h1{font-size:60px}.layered-depth .device.main{right:66px;top:180px}.layered-depth .device.back{right:300px;top:320px;transform:rotate(-12deg);opacity:.66;filter:saturate(.75);z-index:5}.layered-depth .glass{right:250px;top:185px;width:260px;height:170px}
.floating-orbit .copy{left:68px;top:680px;width:850px}.floating-orbit .copy h1{font-size:63px}.floating-orbit .device{right:175px;top:85px;transform:rotate(8deg)}.floating-orbit .orb.a{left:120px;top:210px;width:72px;height:72px}.floating-orbit .orb.b{right:90px;top:530px;width:42px;height:42px;background:#f6b77d}.floating-orbit .orbit{width:760px;height:470px;left:150px;top:130px;transform:rotate(-12deg)}
.macro .copy{left:68px;top:690px;width:880px}.macro .copy h1{font-size:62px}.macro .product-card{left:68px;right:68px;top:120px;height:510px}.macro .product-card img{object-fit:cover;object-position:top center;transform:scale(1.16)}
...
/* Carousel chapters */
.chapter .brand{top:58px}.chapter .copy{left:68px;top:220px;width:860px}.chapter h1{font-size:82px}.chapter .chapter-no{position:absolute;right:58px;top:56px;font:800 170px/.8 Sora;color:rgba(139,99,246,.15)}.chapter .product-card{right:68px;bottom:118px;width:390px;height:270px;transform:rotate(-3deg)}
...
.cta-close .copy{left:68px;top:210px;width:760px}.cta-close h1{font-size:78px}.cta-close .product-card{right:-80px;bottom:-80px;width:580px;height:410px;transform:rotate(-8deg)}.cta-close .cta{font-size:22px;padding:20px 28px}
`;
...
function copy(job, cta = false) {
  const headline = job.visible_copy?.headline || ""; const support = job.visible_copy?.supporting_text || "";
  return `<div class="copy"><div class="eyebrow">${esc(job.post_code?.replace("post_", "Innerbloom · ") || "Innerbloom")}</div><h1>${esc(headline)}</h1>${support ? `<p class="support">${esc(support)}</p>` : ""}${cta ? `<span class="cta">Discover Innerbloom</span>` : ""}</div>`;
}
...
function composition(job, sources) {
  ...
  const cta = /cta|close|early|learn|try/i.test(variant + " " + (job.visible_copy?.headline || ""));
  switch (variant) {
    case "split_device_left": return { cls:"split-left", body:copy(job,cta)+device(a) };
    case "cinematic_device_center": return { cls:"center-stage", body:copy(job,cta)+`<div class="orbit o1"></div><div class="orbit o2"></div>`+device(a) };
    case "device_diagonal_crop": return { cls:"diagonal-crop", body:copy(job,cta)+device(a) };
    case "layered_product_depth": return { cls:"layered-depth", body:copy(job,cta)+`<div class="glass"></div>`+device(b,"small back")+device(a,"main") };
    case "floating_device_orbit": return { cls:"floating-orbit", body:copy(job,cta)+`<div class="orbit"></div><div class="orb a"></div><div class="orb b"></div>`+device(a) };
    case "module_macro_crop": return { cls:"macro", body:copy(job,cta)+card(a) };
    case "bento_product_proof": return { cls:"bento", body:copy(job,cta)+card(b,"a")+card(a,"b")+device(a) };
    case "editorial_type_monument": return { cls:"type-monument", body:copy(job,cta)+`<div class="number">${String(job.slide_number || "01").padStart(2,"0")}</div>` };
    case "editorial_signal_line": return { cls:"signal-line", body:copy(job,cta)+`<div class="signal"><i></i><i></i><i></i><i></i><i></i><i></i></div>` };
    case "editorial_numbered_steps": return { cls:"steps", body:copy(job,cta)+`<div class="step-row"><div class="step"><b>01</b><span>Observe</span></div><div class="step"><b>02</b><span>Adjust</span></div><div class="step"><b>03</b><span>Continue</span></div></div>` };
    case "editorial_quote_frame": return { cls:"quote-frame", body:copy(job,cta)+`<div class="quote">“</div>` };
    case "carousel_chapter_cover": return { cls:"chapter", body:copy(job,false)+`<div class="chapter-no">${String(job.slide_number || 1).padStart(2,"0")}</div>`+card(a) };
    case "carousel_proof_focus": return { cls:"proof-focus", body:copy(job,false)+device(a)+`<div class="badge">Real product proof · no invented UI</div>` };
    case "carousel_transition": return { cls:"transition", body:copy(job,false)+`<div class="orbit"></div><div class="orb"></div>` };
    case "carousel_cta_close": return { cls:"cta-close", body:copy(job,true)+card(a,"landscape") };
    default: return { cls:"split-right", body:`<div class="halo"></div>`+copy(job,cta)+device(a) };
  }
}
