...
const APPROVED_PHASE1_LAYOUTS = new Set([
  "editorial_material_scene","split_device_right","split_device_left","storefront_feature_stage","storefront_metric_overlay","carousel_proof_focus","carousel_cta_close",
]);
...
for (const asset of assets) {
  ...
  let bytes;
  try { bytes = await fs.readFile(pngPath); }
  catch { errors.push(`${code}: PNG is missing`); continue; }
  ...
  if (String(manifest.schema_version || "").includes("phase1") && quality.phase1_collision_pass !== true) errors.push(`${code}: Phase 1 collision retry did not certify the frame`);
  checks.push({asset_code:code,png:path.basename(pngPath),bytes:bytes.length,width,height,sha256:hash,requested_layout_variant:asset.requested_layout_variant || null,effective_layout_variant:asset.layout_variant || null,pilot_scene_asset_key:asset.pilot_scene_asset_key || null,quality,passed:!errors.some(error=>error.startsWith(`${code}:`))});
}

if (manifest.policy?.preview_selects_one_visual_per_post) {
  const sceneKeys = new Set(assets.map(asset => asset.pilot_scene_asset_key).filter(Boolean));
  if (assets.length >= 4 && sceneKeys.size < 4) errors.push(`Phase 1 representative pilot must exercise four approved scene plates; found ${sceneKeys.size}`);
  ...
  let longestRun=0,currentRun=0,previous=null;
  for (const asset of assets) {
    if (asset.layout_variant === previous) currentRun += 1;
    else { previous=asset.layout_variant; currentRun=1; }
    longestRun=Math.max(longestRun,currentRun);
  }
  if (longestRun > 4) errors.push(`Phase 1 campaign repeats one layout ${longestRun} times consecutively; maximum is four`);
}

const report={schema_version:"2.1-phase1",gate:"rendered_creative_v3_phase1",passed:errors.length===0,asset_count:assets.length,thresholds:{copy_product_overlap_px:MAX_COPY_PRODUCT_OVERLAP_PX,copy_support_overlap_px:MAX_COPY_SUPPORT_OVERLAP_PX},checks,errors};
