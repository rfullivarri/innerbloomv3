...
## Visible copy purity
Visible copy fields are final user-facing social copy only. Do not put internal creative instructions, proof logic, composition notes, slide role explanations, source-reference notes, or generation notes inside:
...
Move instructions such as proof logic, visual rationale, composition differences, screenshot usage, and generation notes into `visual_strategy`, `visual_proof_requirement`, `composition_spec`, `generation_prompt`, or `acceptance_criteria`. User-facing visible copy should read like polished campaign copy that can appear directly on the final image.

Invalid visible copy examples include internal lines such as “Show dashboard progress as the proof…”, “Use Daily Energy to prove…”, “Slide 1 changes composition…”, and “registered screenshot…”. Prefer user-facing lines such as “Progress should stay visible, even after an imperfect day.” or “Perfect days are a fragile plan.”
...
- visible copy audit: no visible copy field may contain internal words or phrases like `proof`, `composition`, `visual role`, `serve the role`, `registered screenshot`, `generation`, `use Daily Energy to prove`, `show dashboard progress`, or `claim supported`, unless the phrase is clearly user-facing marketing copy rather than an internal instruction;
